# Trabajo Práctico 4 Resuelto

(trabajo-practico-4-ejercicio-1)=

## Ejercicio 1

Se tiene un sistema operativo con una memoria virtual de 16 TB, con una memoria real de 4 GB y páginas de 32 KB, donde cada dirección corresponde a un byte.
...
memoria real 16tb-> 1tb = 2^40, 16tb = 2^44
memoria virtual 44 bits para poder direccionarla, esto es igual a 11 dígitos hexadecimales

3F 0EE EA0 377

hay 11 digitos hexa, el 3 de la izquierda tiene todos los bits en 1 (usados), con el patrón 0011

se mira direc de mem real, de 4 gb, 4gb = 2^32, osea mem real necesita 32 bits para ser direccionada, 32 bits es 8 dígitos hexadecimales

1 B68 377, 7 dígitos hexa, si bien está correct escrito, se dbee escribir como 01 b68 377

tam pagina es 32 kb, 1 k = 2^10, 2k = 2^11, 32k = 2^15, pag requiere 11 bits

ESTÁN TODOS LOS NECESARIOS PARA RESOLVER PROBLEMA

3F 0EE EA0 377 0011

1 B68 377 91 B68 377 0011 1111-0000 1110 1110-1110 1010 0 **000-0011 0111 0111** <- direc virtual de byte que se quiere ver, 15 bits dercha numero interno pagina

                      0 0111 1110 0001 1101 1101 0101 0100 07E1DDD4

                01B6837 0000 0001 1011 0110 1000 0011 0111

                      0 0000 0011 0110 003E6

cntidad de bits dada por numero de pagina, numero de bits de direccion dado por tamaño de mem (real o virtual), tam pagina ultuimos bits
...
donde cada componente del arreglo ocupa 8 bytes.
...
En el mismo sistema operativo del {ref}`trabajo-practico-4-ejercicio-1` se dispone de una tabla de conversiones TLB de 32 filas.
...
En el mismo sistema operativo del {ref}`trabajo-practico-4-ejercicio-1` se tienen dos procesos que comparten una matriz de números reales de $64 x 64$. En el **proceso 1** la matriz comienza en el primer byte de la página virtual `1AEED1BA`, y en el **proceso 2** en el primer byte de la página virtual `04CC8B20`. Dicha matriz comienza en el primer byte de la página real `346B0`. Suponiendo además que esta matriz se encuentra ubicada en páginas virtuales y reales contiguas:
...
Considerar un sistema operativo con una memoria virtual con páginas de 4 KB. Cada dirección refiere a un byte. En el mismo existe un proceso que posee una matriz de números reales de 8 B de 60 filas por 180 columnas. Se sabe además que esa matriz comienza su almacenamiento en la dirección virtual `5AFAF788`.
...
Considerar una computadora con un CPU capaz de acceder a una memoria de 64K direcciones de 48 B cada una. El mismo posee una unidad de expansión de direccionamiento que permite direccionar 24 MB. Esta unidad divide la memoria en páginas de 12 KB. El sistema operativo ocupa las primeras 2 páginas reales.

En un cierto instante se encuentran alojados tres procesos, que ocupan 45 KB, 33 KB y 53 KB. El **proceso 1** tiene una asignadas las páginas reales $9$, $22$, $40$ y siguientes, el **proceso 2** tiene asignadas las páginas reales $30$ y las siguientes. finalmente el **proceso 3** tiene asignada las páginas reales $12$, $6$, $7$, $57$ y siguientes.
